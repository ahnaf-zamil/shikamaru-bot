...
class Fun(lightbulb.Plugin):
	...
	async def get_meme(self):
		async with aiohttp.ClientSession() as session:
			sub_reddit = self.bot.config['meme_subreddit'].replace("https://reddit.com/r/", "")
			r = await fetch(session, "https://meme-api.herokuapp.com/gimme/" + sub_reddit)
			r = json.loads(r)
	...
	async def get_joke(self):
		async with aiohttp.ClientSession() as session:
			r = await fetch(session, "https://official-joke-api.appspot.com/jokes/random")
			r = json.loads(r)
		return r 
	...
	async def meme(self, ctx):
		...
		em.set_author(name=f"By {json_data['author']}", url="https://www.reddit.com/user/" + json_data["author"])
	...
	async def cat(self, ctx):
		...
		async with aiohttp.ClientSession() as session:
			async with session.get('https://api.thecatapi.com/v1/images/search') as resp:
				...
				js = await resp.json()
				await ctx.reply(embed=hikari.Embed(title=random.choice(cat_titles), color="#dffc03").set_image(js[0]['url']))
	...
	async def fortune(self, ctx, *, question: str = None):
		...
		if question:
			...
			await ctx.reply(
                f"{ctx.message.author.mention}, {random.choice(responses)}"
			)
			return
